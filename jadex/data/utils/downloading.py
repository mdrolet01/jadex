...
from jadex.utils.printing import print_red

USER_AGENT = "pytorch/vision"


def _urlretrieve(
    url: str, filename: Union[str, pathlib.Path], chunk_size: int = 1024 * 32, verify: bool = True
) -> None:
    # Create SSL context
    context = ssl.create_default_context()
    if not verify:
        print_red(
            "WARNING: SSL certificate verification is disabled. Proceeding with insecure download from:\n"
            + f"{url}\n"
            + "Press Ctrl+C to cancel if this is unexpected (will proceed in 10 seconds)."
        )
        time.sleep(10)
    ...
    with urllib.request.urlopen(req, context=context) as response:
        ...
        with open(filename, "wb") as fh, tqdm(total=total, unit="B", unit_scale=True) as pbar:
...
_ARCHIVE_EXTRACTORS: Dict[
    str, Callable[[Union[str, pathlib.Path], Union[str, pathlib.Path], Optional[str]], None]
] = {
    ".tar": _extract_tar,
    ".zip": _extract_zip,
}
...
def _decompress(
    ...
    if to_path is None:
        to_path = pathlib.Path(
            os.fspath(from_path).replace(suffix, archive_type if archive_type is not None else "")
        )
...
def flip_byte_order(arr: np.ndarray) -> np.ndarray:
    byte_view = arr.view(np.uint8)  # interpret data as bytes
    itemsize = arr.itemsize
    new_shape = arr.shape + (itemsize,)

    # Reshape to (original_shape..., bytes_per_item), then flip the last axis
    flipped_bytes = byte_view.reshape(new_shape)[..., ::-1]

    # Return to original dtype
    return flipped_bytes.reshape(arr.shape).view(arr.dtype)
