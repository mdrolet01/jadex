...
from jadex.data.utils.downloading import check_integrity, download_and_extract_archive
...
class CIFARDataset(CIFAR10):
    def __init__(self, cfg, mode, ctx=None):
        ...
        assert cfg.dataset.height == cfg.dataset.width == 32, "only 32x32 size supported for CIFAR"
        assert cfg.dataset.channels == 3, "only 3 channels supported for CIFAR"
        self.cfg = cfg
        data_dir = CACHE_DIR / "cifar10"
        data_dir.mkdir(parents=True, exist_ok=True)
        CIFAR10.__init__(self, root=data_dir, train=bool(mode == "train"), download=True)
    ...
    @property
    def mean(self):
        return np.array([125.307, 122.950, 113.865])

    @property
    def std(self):
        return np.array([62.993, 62.089, 66.705])
