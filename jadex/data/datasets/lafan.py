...
class LAFANDataset(BaseDataset):
    def __init__(self, cfg, mode, ctx):
        ...
        h1rel_feature_cond = (
            cfg.env.env_name == "UnitreeH1"
            and cfg.spaces.input.params.use_positions
            and cfg.spaces.input.params.use_rotations
            and not cfg.spaces.input.params.use_velocities
        )

        assert h1rel_feature_cond, "Only this config is supported for vae training"
        ...
        # NOTE: we should probably check this more carefully
        ...
        assert len(cfg.task_factory.params.lafan1_dataset_conf.dataset_name) == 36
        assert len(cfg.val_dataset) == 4

        if not (raw_train_data_path.exists() and raw_val_data_path.exists()):
            print_green("Creating LAFAN dataset...")
            from jadex.utils.lafan_utils import RelSites
            from loco_mujoco.task_factories.base import TaskFactory

            factory = TaskFactory.get_factory_cls(cfg.task_factory.name)
            env = factory.make(**cfg.env, **cfg.task_factory.params)
            isp = RelSites.create(env, params=cfg.spaces.input.params)
            train_traj_data = env.th.traj.data

            val_env = factory.make(**cfg.env, lafan1_dataset_conf=dict(dataset_name=cfg.val_dataset))
            val_traj_data = val_env.th.traj.data

            raw_train_data = np.array(isp.space2array(isp.extract_from_trajectory(train_traj_data)))
            raw_val_data = np.array(isp.space2array(isp.extract_from_trajectory(val_traj_data)))
            lafan_data_dir.mkdir()

            train_split_points = np.array(train_traj_data.split_points)
            val_split_points = np.array(val_traj_data.split_points)

            np.savez(raw_train_data_path, data=raw_train_data, split_points=train_split_points)
            np.savez(raw_val_data_path, data=raw_val_data, split_points=val_split_points)
        else:
    ...
    def __len__(self) -> int:
        # NOTE: not implemented for this dataset
        return 40
