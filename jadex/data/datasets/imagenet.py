...
from jadex.data.datasets.base_vision_dataset import ImageFolder
from jadex.data.utils.downloading import check_integrity, extract_archive, verify_str_arg
from jadex.data.utils.transforms import CenterCrop, RandomHorizontalFlip, RandomResizedCrop, Resize
...
class ImageNet(ImageFolder):
    ...
    Args:
        ...
        transform (callable, optional): A function/transform that takes in a PIL image, depends on the given loader,
...
class ImageNetDataset(ImageNet):
    def __init__(self, cfg, mode, ctx=None):
        ...
        assert cfg.dataset.height == cfg.dataset.width, "only square images supported"
        assert is_power_of_two(cfg.dataset.height), "image size must be power of 2"
        assert cfg.dataset.channels == 3, "only 3 channels supported for ImageNet"
        self.cfg = cfg
        self.mode = mode
        data_dir = CACHE_DIR / "imagenet"
        data_dir.mkdir(parents=True, exist_ok=True)
        self.image_size = self.get_feature_shape(X)[:2]
        ImageNet.__init__(self, root=data_dir, split="train" if mode == "train" else "val")
    ...
    @property
    def mean(self):
        return np.array([123.095, 115.324, 102.224])

    @property
    def std(self):
        return np.array([68.819, 66.551, 69.687])
    ...
    def get(self, sampler_data, worker_data=None):
        ...
        if self.mode == "train":
            crop_key, flip_key = jax.random.split(rng_key)
            x = RandomResizedCrop(self.image_size)(x, crop_key)
            x = RandomHorizontalFlip()(x, flip_key)
        else:
            x = Resize(self.image_size)(x)
            x = CenterCrop(self.image_size)(x)
...
def load_meta_file(root: Union[str, Path], file: Optional[str] = None) -> Tuple[Dict[str, str], List[str]]:
    ...
    if check_integrity(file):
        meta_data = np.load(file, allow_pickle=True).item()
        return (meta_data["wnid_to_classes"], meta_data["val_wnids"])
    else:
        msg = (
...
def parse_devkit_archive(root: Union[str, Path], file: Optional[str] = None) -> None:
    ...
    with get_tmp_dir() as tmp_dir:
        ...
        np.save(os.path.join(root, META_FILE), {"wnid_to_classes": wnid_to_classes, "val_wnids": val_wnids})
